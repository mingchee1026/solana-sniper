 ...
 */
export const setSelectedDCAInAmount = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (user.selectedOrderType !== 'dca')
      throw new Error(`${LOG_PREFIX}Can set DCA in amount only in dca order.`);
    // --------------------------------------- END -------------------------------------- //
    ...
    // --------------------------------------- END -------------------------------------- //
    // -------- Update user dca in amount and current price of the non SOL token --------- //
    user.selectedDCAInAmount = amount;
    /**
...
export const setSelectedDCAInAmountValidator = {
  body: Joi.object({
