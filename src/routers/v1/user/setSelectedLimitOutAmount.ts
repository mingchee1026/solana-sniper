 ...
 */
export const setSelectedLimitOutAmount = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (user.selectedOrderType !== 'limit')
      throw new Error(`${LOG_PREFIX}Can set out amount only in limit order.`);
    // --------------------------------------- END -------------------------------------- //
    ...
    // --------------------------------------- END -------------------------------------- //
    // -------- Update user limit order out amount and current price of the non SOL token --------- //
    user.selectedLimitOutAmount = amount;
    /**
...
export const setSelectedLimitOutAmountValidator = {
  body: Joi.object({
