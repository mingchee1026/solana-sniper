 ...
 */
export const setSelectedDCAInAmountPerCycle = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (user.selectedOrderType !== 'dca')
      throw new Error(
        `${LOG_PREFIX}Can set DCA inAmount cycle only in dca order.`
      );
    // --------------------------------------- END -------------------------------------- //
    ...
    // --------------------------------------- END -------------------------------------- //
    // -------- Update user DCA inAmount cycle and current price of the non SOL token --------- //
    user.selectedDCAInAmountPerCycle = amount;
    /**
...
export const setSelectedDCAInAmountPerCycleValidator = {
  body: Joi.object({
