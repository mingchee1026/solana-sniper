 ...
 */
export const setSelectedDCACycleSecondsApart = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (user.selectedOrderType !== 'dca')
      throw new Error(`${LOG_PREFIX}Can set DCA cycle only in dca order.`);
    // --------------------------------------- END -------------------------------------- //
    ...
    // --------------------------------------- END -------------------------------------- //
    // -------- Update user DCA cycle and current price of the non SOL token --------- //
    user.selectedDCACycleSecondsApart = amount;
    /**
...
export const setSelectedDCACycleSecondsApartValidator = {
  body: Joi.object({
